...
- si la feature ajoute un module backend dans server/features, appliquer d'abord le skill backend-feature-creator (création complète), puis backend-feature-structure (audit structurel)
...
Avant de coder, invoquer le skill backend-feature-creator pour la création complète (Prisma, Controller, Service, Routes, Validation, Models, tests, API registration). Ensuite, utiliser backend-feature-structure pour l'audit structurel. Règles de base :
